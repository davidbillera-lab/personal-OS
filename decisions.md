...
3. **`OAUTH_CONSENT_PASSCODE` was a Vercel "sensitive" var → unrecoverable.** `vercel env pull` returns `""` for it and the dashboard won't reveal it, so the real passcode couldn't be retrieved. Reset to a new operator-owned value (`MC-Voice-Operator-2026`), stored in `.env.local` line 46 + the MC vault (encrypted credential `OAUTH_CONSENT_PASSCODE`, id `e6a3e7eb`). Note: reading/writing prod env from the agent session is classifier-blocked (correct) — the operator runs `vercel env` in their own terminal.
