...
> 4. **Hermes role overlap — RESOLVED 2026-07-28.** ChatGPT voice is the mobile front door / capture path; Hermes keeps spec drafting + the Telegram digest lane; Claude Code keeps build + persistence; **MC is the only cross-agent control plane** — agents use their assigned tools (Claude↔GitHub, Codex↔candidate commits), but all handoffs, status, questions, decisions, approvals, and audit events pass through MC. The planned Telegram→brain-dump capture sub-project is absorbed by ChatGPT capture. See `decisions.md` 2026-07-28.
>
> **Cross-check round 2 (2026-07-28) — ChatGPT reviewed the relay plan; accepted with two Claude counters:**
> - **Phase 0 redesigned as 0A–0D.** (0A) Dedicated `chatgpt-liaison` read-only identity — never Hermes's key, even for a test (audit purity). The server currently supports only ONE readonly key env var, so per-agent keys are a **micro-build pulled forward** from Phase 1. (0B) Text MCP test from ChatGPT web/desktop: "Use Mission Control only — what's stalled and why?"; pass = correct tool selected, MC-only answers with task refs, no credentials leaked, actor logged as chatgpt-liaison, failures reported not invented. (0C) Desktop voice test via ChatGPT Work/Codex on the rig. (0D) Phone→host test via ChatGPT Remote pairing. **Honesty flag:** the OpenAI product claims behind 0C/0D (Remote pairing; Work/Codex voice inheriting MCP tools) are ChatGPT-reported and unverified by Claude — Phase 0 is the empirical check either way.
> - **Claude counter #1 — availability.** The desktop-host path dies when the rig sleeps; the Vercel endpoint is serverless (the same reason the digest lane went serverless). Don't pre-commit to host-primary: test both surfaces, let results decide, and keep the Vercel path as the rig-down fallback regardless.
> - **Auth ladder.** No long-lived static Bearer in ChatGPT's connector config as production design: short-lived revocable test key (Phase 0) → OAuth 2.1 facade or host-local identity (Phase 1+). All read tools get the MCP `readOnlyHint` annotation.
> - **Relay contract additions (M2/M3 scope).** Rulings are not brain dumps: add `mc_respond_blocker`, `mc_record_decision`, `mc_resolve_approval` as distinct tools, each referencing the exact blocker/task/spec/approval. Queue reads become **leased claims** (state, claimed_by, lease_expires_at, attempt, idempotency_key, correlation_id). Blockers post as **structured JSON** (task_id, question, options w/ ids, recommendation, risk_if_unanswered, evidence_refs) so voice relay never guesses.
> - **Claude counter #2 — handoff authority.** The Hermes bridge is transport/context only, never the official handoff. Since Hermes can't write to MC, the **builder session persists the spec first** (repo + vault, versioned) and builds only against the persisted version — closes the two-sources-of-truth risk without granting Hermes write.
> - **Phase wording + approvals.** Phase 3: "a user-started Claude Code session claims the approved queued task"; Phase 4: "the managed runner launches a session for an approved READY task." Approvals split four ways (candidate / merge / deploy / migration); **merge-to-main on an auto-deploying Vercel project is a production-tier action**; high-risk approvals are action-bound ("approve approval-123" against an exact commit + expiry), never a loose spoken yes.
