      ...
      },
      {
        ...
        steps: [
          ...
          },
          {
            ...
            detail: 'A second, separate system emails Nicole PDF drafts of ORIGINAL FICTION written for the channel — six writing lanes, each producing one draft every morning. These never enter the app, never appear in the dashboard, and are never posted automatically. They are review-only drafts: Nicole decides use, hold, revise, or reject.',
            warning: 'Never conflate this with the story digest — different system, different sender. "No PDF today" does not block the channel; "no digest today" is a different problem. BUT if BOTH are missing, that is the useful signal: the writing lanes run on the same machine as the daily crawl at the same 6am trigger, so losing both points at the machine being off rather than either system being broken. Also: original fiction is never presented as a real account or case file — that is a reject, not a revise.',
          },
        ],
      },
      {
